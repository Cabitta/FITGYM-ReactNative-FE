...
export async function authenticateBiometric() {
  try {
    ...
    if (!compatible && !isEnrolled) {
      ...
      return { success: false, error: "Dispositivo sin autenticación configurada" };
    }

    // 2️⃣ Obtener tipos soportados (biometría)
    const tiposBiometricos = await LocalAuthentication.supportedAuthenticationTypesAsync();
    ...
    if (!result.success) {
      ...
      if(result.error=="not_enrolled") {
        return { success: false, error: "No hay métodos de autenticación configurados" };
      }
    ...
    Alert.alert("Sesión no encontrada", "No hay sesión guardada en este dispositivo.");
